...
class Aruco3d():
    def __init__(self):
        ...
        self.camera_matrix = np.loadtxt(
            '/home/thevitorhideki/colcon_ws/src/my_package/my_package/config/cameraMatrix_realsense.txt', delimiter=',')
        self.camera_distortion = np.loadtxt(
            '/home/thevitorhideki/colcon_ws/src/my_package/my_package/config/cameraDistortion_realsense.txt', delimiter=',')
        ...
        if ids is not None:
            for i in range(len(ids)):
                ret = aruco.estimatePoseSingleMarkers(
                    cornersList[i], 6, self.camera_matrix, self.camera_distortion)
    ...
    def drawAruco(self, bgr, result):
        ...
        cv2.line(bgr, (bgr.shape[1]//2, bgr.shape[0]//2),
                 ((bgr.shape[1]//2 + 50), (bgr.shape[0]//2)), (0, 0, 255), 5)
        # Desenha a linha referencia em Y
        cv2.line(bgr, (bgr.shape[1]//2, bgr.shape[0]//2),
                 (bgr.shape[1]//2, (bgr.shape[0]//2 + 50)), (0, 255, 0), 5)

        # -- Desenha um retanculo e exibe Id do marker encontrado
        cv2.drawFrameAxes(bgr, self.camera_matrix, self.camera_distortion,
                          result['rvec'], result['tvec'], 0.03)

        aruco.drawDetectedMarkers(bgr, np.array(
            [result['corners']]), np.array([result['id']]))

        return bgr

    def writeDistance(self, bgr, distancia):
        cv2.putText(bgr, f"Distancia: {distancia:.2f} cm", (10, 30),
